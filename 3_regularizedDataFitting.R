...
## Convert baseline categorical covariates into dummy covariates
dffull_dummy <- dummy_cols(dffull, select_columns = base_covnames,
...
t0 <- Sys.time()
rst1 <- 
  gform_noniter_complete(dffull_dummy, K = K, 
...
t1 <- Sys.time()
rst2 <- 
  gform_noniter_match(dffull_dummy, K = K, J = 10,
                      outcome_name = outcome_name, ymodel = ymodel,
                      outcome_mintime = 0, 
                      censor_name = censor_name, censor_model = censor_model, 
                      censor_mintime = 0, 
...
t2 <- Sys.time()
rst3 <- 
  gform_iter_complete(dffull_dummy, K = K, 
                      id_name = 'id',
                      outcome_name = outcome_name, ymodel = ymodel,
                      outcome_mintime = 0, 
                      intervention_name = intervention_name,
...
t3 <- Sys.time()
rst4 <- 
  gform_iter_match(dffull_dummy, K = K, Js = c(5,5,10,10,20,20),
                   outcome_name = outcome_name, ymodel = ymodel,
                   outcome_mintime = 0, 
                   base_covnames = base_covnames,
                   intervention_name = intervention_name,
...
saveRDS(final_rst, file = sprintf('../Results/regularized_repli_%d.RDS', simuID))